...
import { getConfig, enviarConfirmacao } from '../services/whatsapp'
        ...
        {whatsappLink && (
          <div className="flex gap-2">
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="flex items-center justify-center gap-2 flex-1 py-2.5 rounded-xl bg-green-50 text-green-700 border border-green-200 text-sm font-medium hover:bg-green-100 transition-colors"
            >
            ...
            <button
              onClick={async () => {
                setEnviando(true)
                try {
                  const cfg = await getConfig(studio.id)
                  if (!cfg) { toast.error('Configure a Evolution API em Configurações'); return }
                  await enviarConfirmacao(cfg, ag)
              ...
              disabled={enviando}
              className="flex items-center justify-center gap-2 flex-1 py-2.5 rounded-xl bg-primary-50 text-primary-700 border border-primary-200 text-sm font-medium hover:bg-primary-100 transition-colors disabled:opacity-50"
            >
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8" />
              </svg>
              {enviando ? 'Enviando...' : 'Enviar via API'}
            </button>
          </div>
        )}
